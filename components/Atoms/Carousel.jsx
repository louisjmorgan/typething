      ...
      <Slider {...settings} ref={(s) => setSlider(s)}>
        {cards.map((url, index) => (
          <Box
            ...
            backgroundImage={
              useProgressiveImage(url) ? `radial-gradient(circle, rgba(0,0,0,0.75) 0%, rgba(208,194,99,0.1) 75%), linear-gradient(to bottom, rgba(0,0,0,0.75), transparent 20%), url(${url})` : ''
            }
          />
        ))}
      </Slider>
